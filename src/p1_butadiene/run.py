...
def optimize(
    xyz: str, basis: str, method: str, maxiter: int = 80
) -> tuple[float, str, dict]:
    """BFGS Cartesian optimization using SciPy + PySCF gradients.

    Returns (energy_Ha, xyz_string, meta) where meta includes opt_success,
    nit, and |g|_max on the final geometry.
    """
    ...
    if method == "MP2":
        ...
        e, g = energy_and_grad(mol_eq, method)
    ...
    *,
    ...
    for name, geom0 in GEOMS.items():
        ...
        if method == "MP2":
            ...
            e = float(mp.MP2(mf).run().e_tot)
            meta = dict(meta)
            meta["energy_level"] = "MP2//RHF"
        else:
            e, xyz, meta = optimize(geom, basis, method, maxiter=maxiter)
        energies[name] = e
        opt_meta[name] = meta
        defects = check_p1_molecule(name, xyz)
        topo_defects[name] = defects
        if defects:
            print(f"  TOPOLOGY DEFECTS: {defects}", flush=True)
    ...
    all_topo_ok = all(not d for d in topo_defects.values())
    # MP2 inherits RHF opt flags; require success or gmax below threshold when available
    gmaxes = [
        m["gmax_ha_bohr"]
        for m in opt_meta.values()
        if m.get("gmax_ha_bohr") == m.get("gmax_ha_bohr")  # not NaN
    ]
    max_g = float(max(gmaxes)) if gmaxes else float("nan")
    all_opt_ok = all(m.get("opt_success") for m in opt_meta.values()) or (
        gmaxes and max_g <= 1.5e-4
    )
    de_span = abs(ha_to_kcal(ce1)) + abs(ha_to_kcal(ce2))  # CE scale proxy
    gate = {
        "topology_ok": all_topo_ok,
        "opt_ok": bool(all_opt_ok),
        "max_gmax_ha_bohr": max_g,
        "gmax_limit": 1.5e-4,
        "ce_scale_kcal": de_span,
        "ce_scale_ok": de_span < 50.0,  # CE themselves should be tens of kcal, not 1e2
        "defects": topo_defects,
        "opt_meta": opt_meta,
    }
    gate["passed"] = bool(
        gate["topology_ok"] and gate["opt_ok"] and gate["ce_scale_ok"]
    )
    ...
    row = {
        ...
        "quality_gate": gate,
        "agree": (
            bool(ce1 < 0 and ce2 > 0) if gate["passed"] else None
        ),
    }
